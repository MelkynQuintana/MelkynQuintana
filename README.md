...
**Data & Analysis**
`pandas` `NumPy` `SciPy` `emcee` `TensorFlow` `scikit-learn`

**Visualization & BI**
`Matplotlib` `Astropy` `Power BI`

**Tools & Platforms**
`Git` `Jupyter` `LaTeX` `IRAF` `Linux` `React`

**Methods**
`Bayesian Inference` `MCMC` `Monte Carlo methods` `N-Body Simulations` `Big Data Analysis`
...
`Python` `emcee` `MCMC` `Bayesian Inference` `Astropy` `pandas`
...
|---|---|---|
| Bootcamp Talento Tech — Data Analysis (Advanced) | MinTIC / UI Training | 159 hrs |
| Data-Driven Astronomy | Coursera / Univ. of Sydney | 20 hrs |
